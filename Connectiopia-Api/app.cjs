...
const upload_excel = multer({ storage: storage });
  ...
  if (usertype === "superadmin") {
    ...
    bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
      ...
      pool.getConnection((err, connection) => {
        ...
        const sql =
          "INSERT INTO cmsusers(emailid, password, name, usertype, date) VALUES (?,?,?,?,?)";
        connection.query(
          sql,
          [emailid, hashedPassword, name, usertype, date],
          (err, result) => {
            connection.release();
            if (err) {
              console.error(err);
              return res
                .status(500)
                .json({ message: "Error inserting data: " + err });
            }
  ...
  if (usertype === "admin" || usertype === "user") {
    ...
    bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
      ...
      pool.getConnection((err, connection) => {
        ...
        const sqlInsertUser =
          "INSERT INTO cmsusers(emailid, password, organisation, name, usertype, date) VALUES (?,?,?,?,?,?)";

        connection.query(
          sqlInsertUser,
          [emailid, hashedPassword, organisation, name, usertype, date],
          (err, result) => {
            connection.release();
            if (err) {
              console.error(err);
              return res
                .status(500)
                .json({ message: "Error inserting user data: " + err });
            }
      ...
      return multer({
        ...
        fileFilter: function (req, file, cb) {
          ...
          if (allowedMimes.includes(file.mimetype)) {
            ...
            cb(
              new Error("Only jpeg, jpg, png, and svg files are allowed"),
              false
            );
          }
        },
...
app.delete("/delete-image/:campaignid/:pageno/:key", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(
        "Successfully deleted the image from S3 and its data from AWS RDS."
      );
  } catch (error) {
...
function deleteImageData(campaignid, pageno, key) {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      let sql =
        "DELETE FROM campaign_config where campaignid = ? AND pageno = ? AND `key` = ?";
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(sql, [emailid], (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error fetching user data: " + err });
      }
        ...
        if (user.usertype === "admin" || user.usertype === "user") {
          ...
          connection.query(campaignQuery, [emailid], (err, campaignResults) => {
            ...
            if (err) {
              ...
              return res
                .status(500)
                .json({ message: "Error fetching campaign data: " + err });
            }
            ...
            const token = jwt.sign({ emailid: emailid }, JWT_SECRET, {
              expiresIn: "1h",
            });
...
app.post(
  "/uploadgif/:campaignid/:pageno/:Key/:scantype",
  upload("gif").single("image"),
  async (req, res) => {
    ...
    try {
      ...
      pool.getConnection((err, connection) => {
        ...
        const sql =
          "INSERT INTO campaign_config(campaignid,pageno,`key`,scantype) VALUES(?,?,?,?)";
        connection.query(
          sql,
          [campaignid, pageno, key, scantype],
          (err, result) => {
            connection.release();
            if (err) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      pool.getConnection((err, connection) => {
        ...
        connection.query(
          sql,
          [campaignid, pageno, compositeKey, scantype],
          (err, result) => {
            connection.release();
            if (err) {
              console.error("Database error:", err);
...
app.get("/allsignedurls/:campaignid/:scantype", async (req, res) => {
  try {
    ...
    const endDate = moment(results[0].formatted_enddate, "YYYY-MM-DD").endOf(
      "day"
    );

    if (
      results[0].status === "inactive" ||
      !currentDate.isBetween(startDate, endDate, undefined, "[]")
    ) {
    ...
    const signedURLs = await Promise.all(
      keysData.map((data) => getPresignedUrl(campaignid, data.pageno, data.key))
    );
    const response = keysData.map((data, index) => ({
      pageno: data.pageno,
      key: data.key,
      value: signedURLs[index],
    }));
    ...
    const groupedResponse = response.reduce((accumulator, current) => {
      (accumulator[current.pageno] = accumulator[current.pageno] || []).push({
        key: current.key,
        value: current.value,
      });
  ...
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        const checksql =
          'SELECT DATE_FORMAT(startdate, "%Y-%m-%d") AS formatted_startdate,DATE_FORMAT(enddate, "%Y-%m-%d") AS formatted_enddate, status FROM campaign_table WHERE campaignid = ?';
...
function fetchKeysFromDB2(campaignid) {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      const sql =
        "SELECT `key`, `pageno` FROM campaign_config WHERE campaignid = ?";
      connection.query(sql, [campaignid], (err, results) => {
        connection.release();
        if (err) {
          reject(err);
...
function fetchKeysFromDB1(campaignid, scantype) {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      const sql =
        "SELECT `key`, `pageno` FROM campaign_config WHERE campaignid = ? AND scantype = ?";
      connection.query(sql, [campaignid, scantype], (err, results) => {
        connection.release();
        if (err) {
          reject(err);
...
const checkkeyinDB = (campaignid, pageno, compositeKey) => {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      const sql =
        "SELECT COUNT(*) as count FROM campaign_config WHERE campaignid = ? AND pageno = ? AND `key` = ?";
      connection.query(
        sql,
        [campaignid, pageno, compositeKey],
        (err, result) => {
          connection.release();
          if (err) {
            reject(err);
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql, [hashedNewPassword, emailid], (err, results) => {
        ...
        if (results.affectedRows === 0) {
          return res
            .status(400)
            .send("No user found with the provided email ID");
        }
...
app.get("/campaigndetails/:emailid/:usertype", (req, res) => {
  ...
  if (usertype === "superadmin") {
    pool.getConnection((err, connection) => {
      ...
      const sql =
        "SELECT campaignid,organisation,scantype,campaign_name,emailid FROM campaign_table";
      connection.query(sql, emailid, (err, results) => {
        connection.release();
        if (err) {
          res.status(400).json({ err });
  ...
  if (usertype === "admin" || usertype === "user") {
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql1, [emailid], (err, results1) => {
        ...
        const sql2 =
          "SELECT campaignid,scantype,campaign_name,emailid FROM campaign_table WHERE organisation = ?";
...
app.post("/organisation", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    let checkSql =
      "SELECT * FROM organisation WHERE LOWER(organisation) = LOWER(?)";
    connection.query(checkSql, [organisation], (err, results) => {
      ...
      if (results.length > 0) {
        ...
        return res
          .status(400)
          .json({ message: "Organisation already exists." });
      } else {
        // Insert organisation if not exists
        let insertSql =
          "INSERT INTO organisation (organisation, `desc`, createddate) VALUES (?, ?, ?)";
        connection.query(
          insertSql,
          [organisation, desc, date],
          (err, results) => {
            connection.release();
            if (err) {
              return res
  ...
  async (req, res) => {
    ...
    try {
      ...
      pool.getConnection(async (err, connection) => {
        ...
        connection.query(
          checkSql,
          [campaignid, pageno, compositeKeyMind],
          async (err, results) => {
            if (err) {
              console.error("Database check error:", err);
              connection.release();
            ...
            const handleUploadAndInsert = async (s3Params, dbKey) => {
              ...
              await new Promise((resolve, reject) => {
                connection.query(
                  insertSql,
                  [campaignid, pageno, dbKey, scantype],
                  (err, result) => {
                    if (err) reject(err);
                    else resolve(result);
                  }
            ...
            if (results[0].count === 0) {
              ...
              try {
                ...
                res
                  .status(200)
                  .send("Both .mind and image file got uploaded successfully");
              } catch (e) {
                console.error("Upload or Database Insert Error:", e);
                res
                  .status(500)
                  .send(
                    "Error handling uploads or database insert: " + e.message
                  );
              }
            } else {
              // If image already exists in the database, overwrite in S3
              ...
              try {
                ...
                res
                  .status(200)
                  .send(
                    "Both .mind and original image file got overridden in S3"
                  );
              } catch (e) {
                console.error("Upload Error:", e);
                res
                  .status(500)
                  .send("Error overriding files in S3: " + e.message);
              }
            }
            connection.release();
          }
...
app.post("/setStatus", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    let updateSQL =
      "UPDATE campaign_table SET status = ? WHERE campaign_name = ?";
...
app.get("/organisation/:name", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    let sql =
      'SELECT campaign_name, campaignid, scantype, `desc`, DATE_FORMAT(startdate, "%Y-%m-%d") AS startdate, DATE_FORMAT(enddate, "%Y-%m-%d") AS enddate, status FROM campaign_table WHERE organisation = ?';
    ...
    connection.query(sql, [name], async (err, campaigns) => {
      ...
      const promises = campaigns.map(async (campaign) => {
        const sqlEmailsAndUserTypes = `
                    ...
                    WHERE cu.campaignid = ?`;
        const [usersResults] = await connection
          .promise()
          .query(sqlEmailsAndUserTypes, [campaign.campaignid]);
        campaign.users = usersResults;
        return campaign;
      });

      Promise.all(promises)
        .then((resultsWithUsers) => {
          connection.release();
        ...
        .catch((err) => {
          ...
          return res
            .status(500)
            .send("error fetching associated emails and user types");
        });
    });
  });
});
...
app.get("/campaignsByEmailid/:emailid", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    // First, fetch all campaignid values associated with the provided emailid
    const sqlCampaignIds =
      "SELECT campaignid FROM campaign_users WHERE emailid = ?";

    connection.query(
      sqlCampaignIds,
      [emailid],
      async (err, campaignsAssociated) => {
        if (err) {
          connection.release();
          return res.status(500).send("error fetching associated campaigns");
        }
        ...
        // For each campaignid, fetch campaign details and associated users
        const promises = campaignIds.map(async (campaignId) => {
          const sqlCampaignDetails =
            'SELECT campaign_name, campaignid, scantype, status, `desc`, DATE_FORMAT(startdate, "%Y-%m-%d") AS startdate, DATE_FORMAT(enddate, "%Y-%m-%d") AS enddate FROM campaign_table WHERE campaignid = ?';
          const [campaign] = await connection
            .promise()
            .query(sqlCampaignDetails, [campaignId]);

          const sqlAssociatedUsers =
            "SELECT cu.emailid, cms.usertype, cms.name FROM campaign_users cu JOIN cmsusers cms ON cu.emailid = cms.emailid WHERE cu.campaignid = ?";
          const [users] = await connection
            .promise()
            .query(sqlAssociatedUsers, [campaignId]);
        ...
        Promise.all(promises)
          ...
          .catch((err) => {
            ...
            return res
              .status(500)
              .send("error fetching campaign details and associated users");
          });
      }
    );
  });
...
app.post("/editCampaign", (req, res) => {
  const { startdate, enddate, desc, campaign_name, newcampaign_name } =
    req.body;

  if (!campaign_name) {
    return res
      .status(400)
      .json({ error: "Campaign name is required to update." });
  }
  ...
  let sql = `UPDATE campaign_table SET ${updateFields.join(
    ", "
  )} WHERE campaign_name = ?`;
...
app.delete("/deleteCampaign/:campaign_name", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    const sqlFetchCampaignId =
      "SELECT campaignid FROM campaign_table WHERE campaign_name = ?";
    connection.query(
      sqlFetchCampaignId,
      [campaign_name],
      async (err, results) => {
        if (err || results.length === 0) {
          ...
          return res.status(500).json({
            message: "Error fetching campaign or campaign not found: " + err,
          });
        }
        ...
        connection.beginTransaction((err) => {
          ...
          const deleteFromCampaignConfig =
            "DELETE FROM campaign_config WHERE campaignid = ?";
          connection.query(
            deleteFromCampaignConfig,
            [campaignId],
            (err, _results) => {
              if (err) {
                return connection.rollback(() => {
                  connection.release();
                  res.status(500).json({
                    message: "Error deleting from campaign_config: " + err,
                  });
                });
              }

              const deleteFromCampaignUsers =
                "DELETE FROM campaign_users WHERE campaignid = ?";
              connection.query(
                deleteFromCampaignUsers,
                [campaignId],
                (err, _results) => {
                  if (err) {
                    return connection.rollback(() => {
                      connection.release();
                      res.status(500).json({
                        message: "Error deleting from campaign_users: " + err,
                      });
                    });
                  }

                  const deleteFromCampaignTable =
                    "DELETE FROM campaign_table WHERE campaignid = ?";
                  connection.query(
                    deleteFromCampaignTable,
                    [campaignId],
                    (err, _results) => {
                      if (err) {
                        return connection.rollback(() => {
                          ...
                          res.status(500).json({
                            message:
                              "Error deleting from campaign_table: " + err,
                          });
                        });
                      }
                      ...
                      connection.commit(async (err) => {
                        if (err) {
                          return connection.rollback(() => {
                            connection.release();
                            res.status(500).json({
                              message: "Error committing transaction: " + err,
                            });
                          });
                        }
                        ...
                        try {
                          ...
                          return res.status(500).json({
                            message:
                              "Error deleting campaign folder from S3: " +
                              S3Err,
                          });
                        }
                        ...
                        res.status(200).json({
                          message:
                            "Campaign and associated entries deleted successfully",
                        });
                      });
                    }
                  );
    ...
    connection.query(sql, queryParams, (err, results) => {
      if (err) {
        ...
        if (newemailid) {
          ...
          connection.query(
            updateCampaignUsersSQL,
            [newemailid, oldemailid],
            (err, campaignUpdateResults) => {
              ...
              if (err) {
                ...
                return res.status(200).json({
                  message: "Data updated successfully in both tables",
                });
              }
            }
          );
          ...
          return res
            .status(200)
            .json({ message: "Data updated successfully in cmsusers table" });
        }
      }
    });
  });
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      updateOrganisationQuery +=
        updateValues.join(",") + " WHERE organisation = ?";
        ...
        () => {
          ...
          if (neworganisation) {
            ...
            connection.commit((err) => {
              ...
              connection.release();
              res.status(200).json({
                message: "Organisation description updated successfully.",
              });
            });
          }
        }
...
app.post("/assignCampaignToUser", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(querySql, [campaign_name], (err, response) => {
      ...
      let insertSql =
        "INSERT INTO campaign_users(emailid,campaignid) VALUES(?,?)";
...
app.delete("/removeCampaignFromUser", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(querySql, [campaign_name], (err, response) => {
      ...
      let checkSql =
        "SELECT * FROM campaign_users WHERE emailid = ? AND campaignid = ? ";
      ...
      connection.query(checkSql, [emailid, campaignid], (err, responses) => {
        ...
        if (responses.length === 0) {
          return res
            .status(400)
            .send("user is not associated with the campaign");
        }
        // res.status(200).send('New campaign assigned to the user');

        let deleteSql =
          "DELETE FROM campaign_users WHERE emailid = ? AND campaignid = ?";
...
app.delete("/deleteOrganizationData/:organization_name", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      const sqlFetchCampaignIds =
        "SELECT campaignid FROM campaign_table WHERE organisation = ?";
      connection.query(
        sqlFetchCampaignIds,
        [organization],
        (err, campaigns) => {
          if (err) {
            ...
            return res
              .status(500)
              .send("Error fetching campaign IDs: " + err.message);
          }
          ...
          conditionalQueries.push({
            sql: "DELETE FROM organisation WHERE organisation = ?",
            values: [organization],
          });
          ...
          if (hasCampaigns) {
            ...
            conditionalQueries.push({
              sql: `DELETE FROM campaign_table WHERE campaignid IN (${campaignIdPlaceholder})`,
            });
            conditionalQueries.push({
              sql: `DELETE FROM campaign_config WHERE campaignid IN (${campaignIdPlaceholder})`,
            });
            conditionalQueries.push({
              sql: `DELETE FROM custdata WHERE campaignid IN (${campaignIdPlaceholder})`,
            });
            conditionalQueries.push({
              sql: `DELETE FROM campaign_users WHERE campaignid IN (${campaignIdPlaceholder})`,
            });
          }

          conditionalQueries.push({
            sql: "DELETE FROM cmsusers WHERE organisation = ?",
            values: [organization],
          });
                  ...
                  if (s3Error) {
                    return connection.rollback(() => {
                      ...
                      return res
                        .status(500)
                        .send(
                          "Error deleting files from S3: " + s3Error.message
                        );
                    });
                  }
                  finishTransaction();
            ...
            connection.query(query.sql, query.values, (err, result) => {
              if (err) {
                return connection.rollback(() => {
                  ...
                  return res
                    .status(500)
                    .send("Error occurred during transaction: " + err.message);
                });
              }
              queryIndex++;
              executeQuery();
          ...
          const deleteFromS3 = (campaignIds, callback) => {
            ...
            campaignIds.forEach((campaignId) => {
              ...
              S3.listObjectsV2(s3DeleteParams, function (listErr, listData) {
                ...
                const objectsToDelete = listData.Contents.map((content) => ({
                  Key: content.Key,
                }));
            ...
            connection.commit((err) => {
              if (err) {
                return connection.rollback(() => {
                  ...
                  return res
                    .status(500)
                    .send("Transaction commit error: " + err.message);
                });
              }
              connection.release();
              return res
                .status(200)
                .send("Data deletion completed successfully.");
            });
          };
...
app.post("/addPlayerDetails", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    const getCampaignName =
      "SELECT campaign_name, organisation FROM campaign_table WHERE campaignid = ?";
    connection.query(getCampaignName, [campaignid], (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error fetching the data: " + err });
      }
      ...
      if (results && results.length > 0) {
        ...
        let insertColumns =
          "phonenumber, name, campaignid, date, campaign_name, organisation";
        ...
        connection.query(insertSql, queryValues, (err, results) => {
          ...
          if (err) {
            ...
            return res
              .status(500)
              .json({ message: "Error inserting the data: " + err });
          }
        ...
        return res.status(404).json({
          message: `No organisation and campaign found for the given campaign ID: ${campaignid}`,
        });
      }
    });
  });
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const signedURLs = await Promise.all(
        keysData.map((data) =>
          getPresignedUrl(campaignid, data.pageno, data.key)
        )
      );
      const response = keysData.map((data, index) => ({
        pageno: data.pageno,
...
app.post("/api/createNewCampaign", (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    if (usertype == "superadmin") {
      const status = "inactive";
      let sql =
        "INSERT INTO campaign_table(campaignid, organisation, campaign_name, startdate, enddate, `desc`, status, scantype) VALUES(?,?,?,?,?,?,?,?)";
      connection.query(
        sql,
        [
          campaignid,
          ...
          (err, results) => {
            if (err) {
              return connection.rollback(() => {
                ...
                res.status(500).json({
                  message:
                    "Error inserting the data into campaign_table : " + err,
                });
              });
            }
            // return res.status(200).json({ message: 'Campaign created successfully' });
              ...
              (err, results) => {
                if (err) {
                  return connection.rollback(() => {
                    ...
                    res.status(500).json({
                      message:
                        "Error inserting data into campaign_users: " + err,
                    });
                  });
                }
                connection.commit(async (err) => {
    ...
    try {
      ...
      if (keyExists) {
        ...
        res
          .status(200)
          .send(
            "Existing image replaced and database updated with new file extension."
          );
      } else {
        ...
        res
          .status(200)
          .send(
            "Image uploaded and new key stored in database with file extension."
          );
      }
    } catch (error) {
      console.error("Error while uploading:", error);
  ...
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        const sql =
          "SELECT COUNT(*) as count FROM campaign_config WHERE campaignid = ? AND pageno = ? AND `key` LIKE ?";
        connection.query(
          sql,
          [campaignid, pageno, key + "%"],
          (err, result) => {
            connection.release();
            if (err) {
              reject(err);
  ...
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        const sql =
          "UPDATE campaign_config SET `key` = ?, scantype = ? WHERE campaignid = ? AND pageno = ? AND `key` LIKE ?";
        connection.query(
          sql,
          [`${key}.${fileExtension}`, scantype, campaignid, pageno, key + "%"],
          (err, result) => {
            connection.release();
            if (err) {
              reject(err);
...
// POST endpoint to upload Excel sheet and update database
app.post("/upload", upload_excel.single("excelFile"), async (req, res) => {
  try {
